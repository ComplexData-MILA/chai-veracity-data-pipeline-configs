#!/bin/bash
#SBATCH --job-name=cdl-compare-ds
...
# Launch dataset comparison script
BG_PIDS=()
...
uv run scripts/analysis/compare_dataset_quality.py \
    ...
    --n-judge-runs 1 \
    --max-concurrency 128 \
    --datasets configs/dataset_compare_chai_veracity.json --output-dir outputs/compare_dataset_quality_chai_veracity_100x1 &
BG_PIDS+=($!)
