...
class BlueskyReactionJetstream(AsyncIterator[dict[str, Any]]):
    """Async iterator over real-time Bluesky reactions (likes + reposts) via Jetstream.

    Yields only create events for app.bsky.feed.like and app.bsky.feed.repost.
    """
    ...
    def __aiter__(self) -> "BlueskyReactionJetstream":
        return self

    async def __aenter__(self) -> "BlueskyReactionJetstream":
        return self
    ...
    async def __anext__(self) -> dict[str, Any]:
        while not self._closed:
            try:
                ...
                reaction = self._parse_event(event)
                if reaction is None:
                    continue
                return reaction
    ...
    def _parse_event(self, event: dict) -> dict[str, Any] | None:
        """Parse a Jetstream event into a reaction record, or None if not a reaction."""
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Exit after one batch instead of looping forever"
    )
    ...
    while True:
        async with S3DataTool().dataset_generator() as dataset_generator:
            ...
            batch_name = f"bsky-jetstream-reactions-{timestamp}"
            print(batch_name)

            async with BlueskyReactionJetstream() as stream:
                await dataset_generator.from_async_iterator(
                    iter_until_timeout(stream, timeout=args.duration_seconds),
                    name=args.dataset_name,
                    batch=batch_name,
                    streaming_configs=S3DataTool.StreamingConfigs(
                        chunk_size=args.chunk_size
                    ),
                    deduplicate_on=["subject_uri", "event_type", "actor_did"],
                )

            if args.single_batch:
                print("Single batch completed, exiting.")
                break
